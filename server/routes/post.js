...
const upload = multer({
  storage: multerS3({
    ...
    bucket: 'dev-t2',
    key(req, file, cb) {
      cb(null, `original/${Date.now()}_${path.basename(file.originalname.replace(/(\s*)/g, ''))}`);
    },
  }),
...
router.post('/images', isLoggedIn, upload.array('images'), async (req, res, next) => {
  try {
    res.json(req.files.map((file) => file.location.replace(/\/original\//, '/thumb/')));
  } catch (error) {
    console.error(error);
...
router.post('/:postId/retweet', isLoggedIn, async (req, res, next) => {
  try {
    ...
    if (exPost) {
      return res.status(403).send('이 글은 이미 리트윗되었습니다.');
    }
